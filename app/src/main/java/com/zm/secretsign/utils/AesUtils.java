...
public class AesUtils {
    ...
    public static String encrypt(String key, String cleartext) {
        ...
        try {
            LogUtil.e("encrypt.key:" + key);
            LogUtil.e("encrypt.before:" + cleartext);
            byte[] result = encrypt(key, cleartext.getBytes());
//            String encryptStr = parseByte2HexStr(result).replace("\n", "").trim();
            String encryptStr = parseByte2HexStr(result);
            LogUtil.e("encrypt.after:" + encryptStr);
            return encryptStr;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static String decrypt(String key, String encrypted) {
        ...
        try {
            LogUtil.e("decrypt.key:" + key);
            LogUtil.e("decrypt.before:" + encrypted);
            byte[] enc = parseHexStr2Byte(encrypted);
            byte[] result = decrypt(key, enc);
            String after = new String(result);
            LogUtil.e("decrypt.after:" + after);
            return after;
        } catch (Exception e) {
            e.printStackTrace();
        }
